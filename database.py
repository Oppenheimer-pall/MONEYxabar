...
class Database:
    ...
    def add_card(self, user_id: int, card_number: str, expiry: str, card_name: str) -> int:
        with self.get_conn() as conn:
            cur = conn.execute(
                "INSERT INTO cards (user_id, card_number, expiry, card_name, balance) VALUES (?, ?, ?, ?, 0)",
    ...
    def save_transaction(self, user_id, card_id, card_number,
        ...
        with self.get_conn() as conn:
            cur = conn.execute(
                "INSERT INTO transactions (user_id, card_id, card_number, amount, fee, receiver, type, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
